...
class Env:
    ...
    def get_state(self, distribution):
        ...
        :return:
        """
        s_1 = torch.sum(get_output_params(self.net.state_dict(), self.args), dim=1).tolist()[:self.args.kind]
        state = distribution + s_1
        return state

    def get_reward(self, w_vicious):
        """
        ...
        reward = 0.
        tmp_output_vicious = get_output_params(w_vicious, self.args)[:self.args.kind]
        tmp_output_global_new = get_output_params(self.w_global_new, self.args)[:self.args.kind]
        distance = 1e2*torch.pairwise_distance(torch.sum(tmp_output_vicious, dim=1), torch.sum(tmp_output_global_new, dim=1))
    ...
    def step(self, action, state):
        ...
        state_next = self.do_action(state[:self.args.kind], action) + \
                     torch.sum(get_output_params(w_vicious, self.args), dim=1).tolist()[:self.args.kind]
        #print("w_vicious:", w_vicious)
        reward, distance, done = self.get_reward(w_vicious)
        if done:
