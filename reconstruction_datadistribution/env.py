...
class Env:
    ...
    def get_state(self, distribution):
        ...
        :return:
        """
        s_1 = torch.mean(get_output_params(self.net.state_dict()), dim=1).tolist()[:judge_index()]
    ...
    def get_reward(self):
        ...
        :return:
        """
        tmp_output_vicious = get_output_params(self.net.state_dict())
        tmp_output_global_new = get_output_params(self.w_global_new)
    ...
    def step(self, action, state):
        ...
        idxs_vicious = reinforce_control_iid_num(dataset=self.data_train, alpha=tmp_state[:self.args.kind], args=args)
        ...
        state_next = [item1 + item2 for item1, item2 in zip(state.tolist()[:self.args.kind], action)] + \
                     [self.args.kind, self.args.data_size] + torch.mean(get_output_params(w_vicious, dim=1))
